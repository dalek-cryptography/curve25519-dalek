...
use crate::{
    ...
    constants::{AFFINE_ODD_MULTIPLES_OF_BASEPOINT, AFFINE_ODD_MULTIPLES_OF_B_SHL_128},
...
pub fn mul(a: &Scalar, A: &EdwardsPoint, b: &Scalar, C: &EdwardsPoint) -> EdwardsPoint {
    ...
    loop {
        ...
        if d_0_naf[i] > 0 {
            t = &t.as_extended() + &table_A.select(d_0_naf[i] as usize);
        } else if d_0_naf[i] < 0 {
            t = &t.as_extended() - &table_A.select(-d_0_naf[i] as usize);
        }

        if e_0_naf[i] > 0 {
            t = &t.as_extended() + &table_B.select(e_0_naf[i] as usize);
        } else if e_0_naf[i] < 0 {
            t = &t.as_extended() - &table_B.select(-e_0_naf[i] as usize);
        }
        ...
        if d_1_naf[i] > 0 {
            t = &t.as_extended() + &table_negC.select(d_1_naf[i] as usize);
        } else if d_1_naf[i] < 0 {
            t = &t.as_extended() - &table_negC.select(-d_1_naf[i] as usize);
        }
