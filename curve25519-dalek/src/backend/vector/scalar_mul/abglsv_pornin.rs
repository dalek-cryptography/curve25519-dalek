...
pub mod spec {
    ...
    pub fn mul(a: &Scalar, A: &EdwardsPoint, b: &Scalar, C: &EdwardsPoint) -> EdwardsPoint {
        ...
        loop {
            ...
            if d_0_naf[i] > 0 {
                Q = &Q + &table_A.select(d_0_naf[i] as usize);
            } else if d_0_naf[i] < 0 {
                Q = &Q - &table_A.select(-d_0_naf[i] as usize);
            }

            if e_0_naf[i] > 0 {
                Q = &Q + &table_B.select(e_0_naf[i] as usize);
            } else if e_0_naf[i] < 0 {
                Q = &Q - &table_B.select(-e_0_naf[i] as usize);
            }

            if e_1_naf[i] > 0 {
                Q = &Q + &table_B_SHL_128.select(e_1_naf[i] as usize);
            } else if e_1_naf[i] < 0 {
                Q = &Q - &table_B_SHL_128.select(-e_1_naf[i] as usize);
            }

            if d_1_naf[i] > 0 {
                Q = &Q + &table_negC.select(d_1_naf[i] as usize);
            } else if d_1_naf[i] < 0 {
                Q = &Q - &table_negC.select(-d_1_naf[i] as usize);
            }
...
mod tests {
    use crate::{
        backend::scalar_mul_abglsv_pornin as mul,
