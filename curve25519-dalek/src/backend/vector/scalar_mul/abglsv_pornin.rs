...
pub mod spec {
    ...
    use crate::backend::vector::avx2::constants::{
        BASEPOINT_ODD_LOOKUP_TABLE, B_SHL_128_ODD_LOOKUP_TABLE,
    };

    #[cfg(feature = "precomputed-tables")]
    #[for_target_feature("avx512ifma")]
    use crate::backend::vector::ifma::constants::{
        BASEPOINT_ODD_LOOKUP_TABLE, B_SHL_128_ODD_LOOKUP_TABLE,
    };
    ...
    pub fn mul(a: &Scalar, A: &EdwardsPoint, b: &Scalar, C: &EdwardsPoint) -> EdwardsPoint {
        ...
        #[cfg(feature = "precomputed-tables")]
        let table_B_SHL_128 = &B_SHL_128_ODD_LOOKUP_TABLE;
        #[cfg(not(feature = "precomputed-tables"))]
        let table_B_SHL_128 =
            &NafLookupTable5::<CachedPoint>::from(&crate::constants::ED25519_BASEPOINT_SHL_128);

        let table_negC = NafLookupTable5::<CachedPoint>::from(&negC);
        ...
        loop {
            ...
            match d_0_naf[i].cmp(&0) {
                Ordering::Greater => Q = &Q + &table_A.select(d_0_naf[i] as usize),
                Ordering::Less => Q = &Q - &table_A.select(-d_0_naf[i] as usize),
                Ordering::Equal => {}
            }

            match e_0_naf[i].cmp(&0) {
                Ordering::Greater => Q = &Q + &table_B.select(e_0_naf[i] as usize),
                Ordering::Less => Q = &Q - &table_B.select(-e_0_naf[i] as usize),
                Ordering::Equal => {}
            }

            match e_1_naf[i].cmp(&0) {
                Ordering::Greater => Q = &Q + &table_B_SHL_128.select(e_1_naf[i] as usize),
                Ordering::Less => Q = &Q - &table_B_SHL_128.select(-e_1_naf[i] as usize),
                Ordering::Equal => {}
            }

            match d_1_naf[i].cmp(&0) {
                Ordering::Greater => Q = &Q + &table_negC.select(d_1_naf[i] as usize),
                Ordering::Less => Q = &Q - &table_negC.select(-d_1_naf[i] as usize),
                Ordering::Equal => {}
            }
...
mod tests {
    use crate::{
        backend::scalar_mul_abglsv_pornin as mul, constants::ED25519_BASEPOINT_POINT,
