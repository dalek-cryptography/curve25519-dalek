...
class CallGraphGenerator:
    def __init__(
        ...
        if not self.graph_tool_path.exists():
            raise FileNotFoundError(
                f"Graph generation tool not found: {graph_tool_path}"
            )
    ...
    def generate_graphs_from_mapping(self, mapping_file, depth=5):
        ...
        results = {"successful": [], "failed": [], "empty": [], "errors": []}
        ...
        for line in lines:
            ...
            try:
                ...
                if func_name and symbol_id:
                    success, reason = self.generate_call_graph_for_function(
                        func_name, symbol_id, depth
                    )
    ...
    def create_summary_report(self, results, output_file):
        ...
        with open(output_file, "w") as f:
            ...
            if results["successful"]:
                f.write(
                    f"### ✅ Successfully Generated Graphs ({len(results['successful'])})\n\n"
                )
            ...
            if results["failed"]:
                f.write(
                    f"### ❌ Failed to Generate Graphs ({len(results['failed'])})\n\n"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call graphs for Verus functions"
    )
    parser.add_argument(
        "graph_tool", help="Path to generate_function_subgraph_dot binary"
    )
    ...
    parser.add_argument(
        "--successful-list", help="Output file for list of successful functions"
    )
    parser.add_argument(
        "--filter-sources",
    ...
    try:
        ...
        # Print summary
        total_attempted = (
            len(results["successful"]) + len(results["empty"]) + len(results["failed"])
        )
        print(
