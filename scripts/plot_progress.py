...
def plot_overall_progress(stats: Dict[str, int], output_dir: Path):
    ...
    percentages = [round(count * 100 / total, 1) if total > 0 else 0 for count in counts]
...
def plot_file_breakdown(df: pd.DataFrame, output_dir: Path):
    ...
    for module in df["module"].unique():
        ...
        total = len(module_df)
        verus_specs = (module_df["has_spec_verus"].notna() & (module_df["has_spec_verus"] != "")).sum()
    ...
    ax.set_ylabel("Number of Functions", fontsize=12, fontweight="bold")
    ax.set_title("Top 15 Modules by Function Count - Verus Progress", fontsize=14, fontweight="bold", pad=20)
    ax.set_xticks(x)
...
def print_summary(stats: Dict[str, int]):
    ...
    print(f"\n{'VERUS':^60}")
    print("-" * 60)
    print(f"  Total with specs:      {stats['verus_specs']:4d} ({round(stats['verus_specs']*100/total, 1):5.1f}%)")
    print(f"    - Full specs:        {stats['verus_specs_full']:4d} ({round(stats['verus_specs_full']*100/total, 1):5.1f}%)")
    print(f"    - External body:     {stats['verus_specs_external']:4d} ({round(stats['verus_specs_external']*100/total, 1):5.1f}%)")
    print(f"  Fully verified:        {stats['verus_proofs']:4d} ({round(stats['verus_proofs']*100/total, 1):5.1f}%)")
    
    print(f"\n{'LEAN':^60}")
    print("-" * 60)
    print(f"  Total with specs:      {stats['lean_specs']:4d} ({round(stats['lean_specs']*100/total, 1):5.1f}%)")
    print(f"  Fully verified:        {stats['lean_proofs']:4d} ({round(stats['lean_proofs']*100/total, 1):5.1f}%)")
    
    print(f"\n{'OVERALL':^60}")
    print("-" * 60)
    print(f"  No specs (any system): {stats['no_specs']:4d} ({round(stats['no_specs']*100/total, 1):5.1f}%)")
    print("=" * 60 + "\n")
