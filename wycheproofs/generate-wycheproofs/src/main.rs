...
mod generator_ed25519 {
    ...
    impl Display for Ed25519tcs {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
            ...
            write!(f, "{}", tcs.into_iter().map(|a| a.1.to_string()).join("\n\n"))
        }
    }
    ...
    impl Display for Ed25519tc {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {    
            ...
            let test_fn = scope.new_fn(&tc_name)
                .attr("test")
                .doc(format!("wycheproofs TC[{}] {}", self.tc_id, self.comment));

            test_fn.line(format!("super::ed25519_dalek_wycheproof(\"{}\", \"{}\", \"{}\")",
    ...
    pub fn run() {
        ...
        let test_set = wycheproof::eddsa::TestSet::load(wycheproof::eddsa::TestName::Ed25519).unwrap();
        for test_group in test_set.test_groups {
            let wycheproof::EddsaPublic { pk, ..  } = test_group.key;
            for test in test_group.tests {

                gen_tests.tc.insert(test.tc_id,
                                    Ed25519tc {
                                        tc_id: test.tc_id,
        ...
        file.write_all(gen_tests.to_string().as_bytes()).expect("Failed to write the generated output.");
