...
impl ECDLPTables26 {
    fn load() -> std::io::Result<Self> {
        ...
        let mut t2_table = vec![T2MontgomeryCoordinates::default(); 256].into_boxed_slice();

        let mmap = true;

        let mut t1_file = File::open("t1_26.bin")?;
        let mut t2_file = File::open("t2_26.bin")?;
...
impl PrecomputedECDLPTables for ECDLPTables26 {
    const L1: usize = 26;
...
pub fn ecdlp_bench(c: &mut Criterion) {
    let tables = ECDLPTables26::load().unwrap();
    ...
    c.bench_function("fast ecdlp find 0", |b| {
        ...
        let point = Scalar::from(num) * G;
        b.iter(|| {
            let res = ecdlp::decode(&tables, black_box(point), ECDLPArguments::new_with_range(0, 1 << 48));
            assert_eq!(res, Some(num as i64));
        });
    });
    ...
    //             &tables,
    //             black_box(point),
    //             ECDLPArguments::new_with_range(0, 1 << 48).n_threads(4),
    //         );
    //         assert_eq!(res, Some(num as i64));
    //     });
    // });

    c.bench_function(&format!("fast ecdlp for number < {}", (1 << 47)), |b| {
    ...
    c.bench_function(&format!("fast ecdlp for number < {}", (1 << 44)), |b| {
    ...
    c.bench_function(&format!("fast ecdlp for number < {}", (1 << 43)), |b| {
