...
def get_commit_history(
    repo_path: Path, branch: str = "main", since_date: str | None = None, max_commits: int = 50
...
def analyze_csv_at_commit(
    ...
    try:
        ...
        if has_verus_proof_col:
            verus_proofs = sum(
                1 for row in rows 
                if row.get("has_proof_verus", "").strip() == "yes"
            )
        ...
        if has_lean_spec_col:
            lean_specs = sum(
                1 for row in rows 
                if row.get("has_spec_lean", "").strip()
            )
        
        if has_lean_proof_col:
            lean_proofs = sum(
                1 for row in rows 
                if row.get("has_proof_lean", "").strip()
            )
...
def collect_historical_data(
    ...
    for i, (commit_hash, commit_date, commit_msg) in enumerate(commits, 1):
        print(f"[{i}/{len(commits)}] {commit_date.strftime('%Y-%m-%d')} {commit_hash[:8]} - {commit_msg}")
...
def print_summary(df: pd.DataFrame):
    ...
    print(f"  With specs:       {first['verus_specs']:4d} ({first['verus_specs']/first['total']*100:5.1f}%)")
    print(f"  With proofs:      {first['verus_proofs']:4d} ({first['verus_proofs']/first['total']*100:5.1f}%)")
    
    print(f"\n{'VERUS - Latest Commit':^70}")
    print("-" * 70)
    print(f"  Total functions:  {last['total']:4d}")
    print(f"  With specs:       {last['verus_specs']:4d} ({last['verus_specs']/last['total']*100:5.1f}%)")
    print(f"  With proofs:      {last['verus_proofs']:4d} ({last['verus_proofs']/last['total']*100:5.1f}%)")
