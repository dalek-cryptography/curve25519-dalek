...
pub open spec fn as_nat_32_u8(bytes: &[u8; 32]) -> nat {
    ...
    (bytes[0] as nat) + (bytes[1] as nat) * pow2(1 * 8) + (bytes[2] as nat) * pow2(2 * 8) + (
    bytes[3] as nat) * pow2(3 * 8) + (bytes[4] as nat) * pow2(4 * 8) + (bytes[5] as nat) * pow2(
        5 * 8,
    ) + (bytes[6] as nat) * pow2(6 * 8) + (bytes[7] as nat) * pow2(7 * 8) + (bytes[8] as nat)
        * pow2(8 * 8) + (bytes[9] as nat) * pow2(9 * 8) + (bytes[10] as nat) * pow2(10 * 8) + (
    ...
    bytes[16] as nat) * pow2(16 * 8) + (bytes[17] as nat) * pow2(17 * 8) + (bytes[18] as nat)
        * pow2(18 * 8) + (bytes[19] as nat) * pow2(19 * 8) + (bytes[20] as nat) * pow2(20 * 8) + (
