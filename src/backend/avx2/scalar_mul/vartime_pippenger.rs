...
/// See documentation in ../../scalar_mul/vartime_pippenger.rs
...
{   
    ...
    multiscalar_mul_window(scalars, points, w)
}

pub(crate) fn multiscalar_mul_window<I, J>(scalars: I, points: J, w: usize) -> EdwardsPoint
where
    I: IntoIterator,
    I::Item: Borrow<Scalar>,
    J: IntoIterator,
...
{   
    ...
    let all_digits: Vec<_> = scalars.into_iter()
        .map(|c| c.borrow().to_arbitrary_radix(w))
        .collect();
    let ps: Vec<ProjectiveNielsPoint> = points.into_iter().map(|p| CachedPoint::from(p.borrow())).collect();
    ...
    for i in (0..windows_count).rev() {
        ...
        let mut buckets:Vec<_> = (0..buckets_count).map(|_| ExtendedPoint::identity()).collect();
...
mod test {
    ...
    fn test_multiscalar_mul() {
        ...
        let points: Vec<_> = (0..n).map(|i| {
            constants::ED25519_BASEPOINT_POINT * Scalar::from_u64(1+i as u64)
        }).collect();
        let scalars: Vec<_> = (0..n).map(|i| Scalar::from_u64((i + 1) as u64).invert()).collect();

        while n > 0 {
            let scalars = &scalars[0..n].to_vec();
            let points = &points[0..n].to_vec();

            let subject = multiscalar_mul(scalars.clone(), points.clone());

            let control = scalars.iter()
                .zip(points.iter())
                .map(|(sc,pt)| sc*pt)
                .fold(EdwardsPoint::identity(), |t,p| t+p);
