...
#[allow(unused_imports)] // Used in verus! blocks
use crate::backend::serial::curve_models::{ProjectivePoint, ProjectiveNielsPoint, AffineNielsPoint};
#[allow(unused_imports)] // Used in verus! blocks
...
pub open spec fn is_valid_projective_point(point: crate::backend::serial::curve_models::ProjectivePoint) -> bool {
    let x = field_element(&point.X);
    let y = field_element(&point.Y);
    let z = field_element(&point.Z);
...
pub open spec fn is_well_formed_edwards_point(point: crate::edwards::EdwardsPoint) -> bool {
    limbs_bounded(&point.X, 54) &&
    limbs_bounded(&point.Y, 54) &&
    limbs_bounded(&point.Z, 54) &&
    limbs_bounded(&point.T, 54)
...
pub open spec fn is_valid_projective_niels_point(
    niels: ProjectiveNielsPoint
) -> bool {
...
pub open spec fn is_valid_affine_niels_point(
    niels: AffineNielsPoint
) -> bool {
    exists|point: EdwardsPoint|
        is_valid_edwards_point(point) &&
