...
def create_gallery(graphs_dir, output_file, title="Verus Function Call Graphs"):
    ...
    with open(output_file, "w") as f:
        ...
        if svg_files:
            f.write(
                "This document contains the call graphs for newly verified Verus functions in this PR.\n\n"
            )
            ...
            for svg_file in svg_files:
                ...
                f.write(
                    f"- [Function: `{func_name}`](#{func_name.lower().replace('_', '-')})\n"
                )
            ...
            for svg_file in svg_files:
                ...
                f.write(
                    "This graph shows the dependency relationships between the Verus-verified function "
                )
                f.write(
                    f"`{func_name}` and libsignal functions, with a depth of 5 levels.\n\n"
                )
            ...
            f.write(
                "- The detected functions don't have paths to libsignal functions\n"
            )
        ...
        f.write(
            "- **Focus**: Paths from Verus-verified functions to libsignal dependencies\n\n"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create a markdown gallery of call graphs"
    )
    ...
    parser.add_argument(
        "--title", default="Verus Function Call Graphs", help="Title for the gallery"
    )
