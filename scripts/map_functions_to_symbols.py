...
class SymbolMapper:
    ...
    def find_symbol_for_function(self, func_name):
        ...
        for s in self.symbols:
            if func_name in s and (
                "impl" in s or "field" in s.lower() or "curve25519" in s.lower()
            ):
...
def main():
    ...
    parser.add_argument(
        "functions_file", help="File containing function names (one per line)"
    )
    parser.add_argument(
        "--output", "-o", help="Output file for mapping (default: stdout)"
    )
    parser.add_argument(
        "--format",
        choices=["pipe", "json"],
    ...
    print(
        f"Successfully mapped {len(mapping)} out of {len(functions)} functions",
        file=sys.stderr,
    )
    return 0
