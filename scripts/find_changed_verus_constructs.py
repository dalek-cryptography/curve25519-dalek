...
def get_git_diff(base_ref="origin/main"):
    ...
    try:
        ...
        print(
            f"DEBUG: Running command: git diff {base_ref}..HEAD --unified=10 -- [... {len(rust_files)} rust files ...]",
            file=sys.stderr,
        )
        ...
        print(
            f"DEBUG: Git diff returned {len(result.stdout)} characters", file=sys.stderr
        )
        if result.stdout:
            print(
                f"DEBUG: First 500 chars of diff:\n{result.stdout[:500]}",
                file=sys.stderr,
            )
...
def extract_function_name(line: str) -> Optional[str]:
    ...
    match = re.search(
        r"(?:pub\s+)?(?:proof\s+|spec\s+|exec\s+|open\s+|closed\s+)?fn\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*[(<]",
        line,
    )
    if match:
...
def has_verus_syntax_in_function(
    lines: List[str], func_name: str, all_functions_in_diff: Dict[str, Tuple[int, bool]]
) -> bool:
...
def extract_changed_verus_constructs(diff_output: str) -> Dict[str, List[str]]:
    ...
    constructs = {"added": set(), "modified": set(), "all": set()}
    ...
    for i, line in enumerate(lines):
        ...
        if func_name:
            ...
            print(
                f"DEBUG: Found function '{func_name}' at line {i} (new={is_new})",
                file=sys.stderr,
            )
    ...
    for i, line in enumerate(lines):
        ...
        for j in range(i, -1, -1):
            ...
            clean_line = (
                check_line[1:]
                if check_line and check_line[0] in ["+", "-", " "]
                else check_line
            )
            func_name = extract_function_name(clean_line)
            ...
            if func_name:
                ...
                print(
                    f"DEBUG: Change at line {i} is inside function '{func_name}'",
                    file=sys.stderr,
                )
                break
        ...
        if enclosing_func:
            ...
            if is_inside_verus_context(lines, i):
                is_verus = True
                print(
                    f"DEBUG: Function '{enclosing_func}' is inside verus! block",
                    file=sys.stderr,
                )
            ...
            if func_def_line is not None and has_verus_annotation(lines, func_def_line):
                is_verus = True
                print(
                    f"DEBUG: Function '{enclosing_func}' has Verus annotations",
                    file=sys.stderr,
                )
            ...
            if func_def_line is not None:
                ...
                clean_func_line = (
                    func_line[1:]
                    if func_line and func_line[0] in ["+", "-", " "]
                    else func_line
                )
                if re.search(r"(?:proof|spec|exec|open|closed)\s+fn", clean_func_line):
                    is_verus = True
                    print(
                        f"DEBUG: Function '{enclosing_func}' is a proof/spec/exec function",
                        file=sys.stderr,
                    )

            # Check 4: Does the function contain Verus-specific syntax?
            if not is_verus and has_verus_syntax_in_function(
                lines, enclosing_func, all_functions_in_diff
            ):
                is_verus = True
                print(
                    f"DEBUG: Function '{enclosing_func}' contains Verus-specific syntax",
                    file=sys.stderr,
                )
    ...
    print(
        f"DEBUG: Summary - Added: {len(result['added'])}, Modified: {len(result['modified'])}, Total: {len(result['all'])}",
        file=sys.stderr,
    )
...
def main():
        ...
        if arg == "--test":
            ...
            if i + 1 < len(sys.argv):
                ...
                try:
                    ...
                    if constructs["all"]:
                        print(
                            f"TEST MODE: Found {len(constructs['all'])} Verus constructs:",
                            file=sys.stderr,
                        )
    ...
    if output_list:
        print(
            f"Found {len(output_list)} Verus constructs ({output_format}):",
            file=sys.stderr,
        )
