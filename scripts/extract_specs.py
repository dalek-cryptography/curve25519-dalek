...
_MATH_SUBSTITUTIONS = [
    ...
    (r"is_square\((\w+)\)", r"\1 is QR mod p"),
    (r"!is_square\(([^)]+)\)", r"\1 is not QR mod p"),
    (r"sqrt_m1\(\)", "i"),
    (r"p\(\)\s*-\s*1", "p - 1"),
    (r"p\(\)", "p"),
    (r"pow\(([^,]+),\s*(\w+)\)", r"(\1)^\2"),
    ...
    (r"fe51_as_canonical_nat\(&?(\w+)\)", r"fe(\1)"),
    (r"fe51_as_canonical_nat_as_nat\(&?(\w+)\)", r"nat(\1)"),
    (r"spec_add_fe51_limbs\((\w+),\s*&?(\w+)\)", r"\1 + \2 (limbs)"),
    (r"u8_32_as_nat\(&?(\w+)(?:\.bytes)?\)", r"nat(\1)"),
    (r"scalar52_to_nat\(&?(\w+)\)", r"nat(\1)"),
    (r"scalar_as_nat\(&?(\w+)\)", r"nat(\1)"),
    (r"group_order\(\)", "L"),
