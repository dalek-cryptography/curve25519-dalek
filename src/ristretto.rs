...
impl ConditionallyAssignable for RistrettoPoint {
    ...
    /// let B = constants::RISTRETTO_ED25519_BASEPOINT;
    ///
    /// let mut P = A;
    ///
...
mod test {
    ...
    fn serde_cbor_basepoint_roundtrip() {
        let output = serde_cbor::to_vec(&constants::RISTRETTO_ED25519_BASEPOINT).unwrap();
        let parsed: RistrettoPoint = serde_cbor::from_slice(&output).unwrap();
        assert_eq!(parsed, constants::RISTRETTO_ED25519_BASEPOINT);
    }

    #[test]
    fn scalarmult_ristrettopoint_works_both_ways() {
        let P = constants::RISTRETTO_ED25519_BASEPOINT;
    ...
    fn decompress_id() {
        ...
        for P in &id.coset4() {
            if P.compress_edwards() == CompressedEdwardsY::identity() {
    ...
    fn basepoint_roundtrip() {
        let bp_compressed_ristretto = constants::RISTRETTO_ED25519_BASEPOINT.compress();
    ...
    fn encodings_of_small_multiples_of_basepoint() {
        ...
        let mut bp = constants::RISTRETTO_ED25519_BASEPOINT;
        for i in 0..16 {
            assert_eq!(bp.compress(), compressed[i]);
            bp = &bp + &constants::RISTRETTO_ED25519_BASEPOINT;
        }
    }
    */
    ...
    fn four_torsion_basepoint() {
        let bp = constants::RISTRETTO_ED25519_BASEPOINT;
    ...
    fn four_torsion_random() {
        ...
        let B = &constants::RISTRETTO_ED25519_BASEPOINT_TABLE;
